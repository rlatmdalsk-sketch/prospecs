...
function CartPage() {
    ...
    if (items.length === 0) {
        return (
            <div className={twMerge("py-40", "text-center")}> 장바구니에 담긴 상품이 없습니다.</div>
        );
    }
            ...
            <div className={twMerge("max-w-7xl", "mx-auto", "px-4", "py-40")}>
                ...
                <div className={twMerge("flex", "gap-10")}>
                    ...
                    <div className={twMerge("flex-1")}>
                        ...
                        {items.map(item => {
                            const prodcut = item.productSize.productColor.product;
                            ...
                            return (
                                <div
                                    ...
                                    <div className={twMerge("w-full", "flex", "gap-4")}>
                                        ...
                                        <div className={twMerge("flex","flex-col","justify-center","gap-1")}>
                                    ...
                                    <div className={twMerge("w-auto","flex","justify-between","items-center")}>
                                        <div className={twMerge("w-24","flex","items-center","border","border-gray-300")}>
                                            <button className={twMerge("w-8","h-full","flex","justify-center","items-center","hover:bg-gray-50")}
                                            onClick={() => updateQuantity(item.id, item.quantity -1)}
                                            >
                                                <FiMinus size={12} />
                                            </button>
                                            <span className={twMerge("flex-1","text-center","text-sm","font-bold")}>
                                                {item.quantity}
                                            </span>
                                            <button className={twMerge("w-8","h-full","flex","justify-center","items-center","hover:bg-gray-50")}
                                                    onClick={() => updateQuantity(item.id, item.quantity +1)}>
                                    ...
                                    <div className={twMerge("w-32","text-center")}>
                                        {(prodcut.price * item.quantity). toLocaleString()}
                                    </div>
                                        <div className={twMerge("text-gray-400","p-2")} onClick={() => removeItem(item.id)}>
                                            <FiX size={20} />
                                        </div>
                                    </div>
                                </div>
