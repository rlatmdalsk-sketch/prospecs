...
import { cancleOrder, getOrderDetail } from "../../api/order.api.ts";
...
function MyOrderDetail() {
    ...
    const handleCancelOrder = async () => {
        ...
        if (window.confirm("정말 취소하시겠습니까?")) {
            try {
                await cancleOrder(order.id, reason);
                alert("주문이 취소되었습니다.");
            ...
            <div className="grid md:grid-cols-2 gap-8">
                <section>
                    ...
                    <div className="border-t border-gray-900 pt-4 text-sm space-y-3">
                        <div className="flex">
                            <span className="w-24 font-bold text-gray-700 flex-shrink-0">받는사람</span>
                            <span className="text-gray-900">{order.recipientName}</span>
                        </div>
                        <div className="flex">
                            <span className="w-24 font-bold text-gray-700 flex-shrink-0">연락처</span>
                ...
                <section>
                    ...
                    <div className="border-t border-gray-900 pt-4 text-sm space-y-3">
                        <div className="flex justify-between">
                            ...
                            <span className="text-gray-900">{order.payment?.method || "무통장입금"}</span>
                        </div>
                        <div className="flex justify-between">
                            <span className="font-bold text-gray-700">상품금액</span>
                            <span className="text-gray-900">{order.totalAmount.toLocaleString()}원</span>
                        </div>
